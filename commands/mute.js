...
.setColor(0x36393f);
  ...
  let whomute = new Discord.RichEmbed()
    .setColor(0x36393f)
    .setDescription('Who must I mute?');
  
  let cantmute = new Discord.RichEmbed()
    .setColor(0x36393f)
    .setDescription('I cannot mute this person!');
  
  
  var mutedmember = message.mentions.members.first(); // sets the mentioned user to the var kickedmember
  if (!mutedmember) return message.channel.send(whomute) // if there is no kickedmmeber var
  if (mutedmember.hasPermission("KICK_MEMBERS")) return message.channel.send(cantmute) // if memebr is an admin
  ...
  if(!muterole){
   ...
   mutedmember.addRole(muterole)
  let muted = new Discord.RichEmbed()
    .setColor(0x36393f)
    .setDescription(`${mutedmember.user} has been muted! \nReason: **${mutereason}**`)
...
exports.help = {
  name: 'mute',
  description: 'Mutes the desired person!',
  usage: 'rmute',
  inHelp: 'yes'
};
